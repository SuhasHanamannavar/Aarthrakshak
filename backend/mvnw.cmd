...
IF NOT EXIST "%MAVEN_PROJECTBASEDIR%\.mvn\wrapper\maven-wrapper.jar" goto endWrapper

@REM Resolve any "." and ".." in MAVEN_PROJECTBASEDIR to make it shorter.
for %%i in ("%MAVEN_PROJECTBASEDIR%") do set MAVEN_PROJECTBASEDIR=%%~fi

@REM Use the maven-wrapper.jar to run Maven, passing -Dmaven.multiModuleProjectDirectory
...
:endWrapper
@REM If maven-wrapper.jar doesn't exist, fall back to system Maven
if "%M2_HOME%"=="" goto systemMaven
...
if exist "%ProgramFiles%\Apache\maven\bin\mvn.cmd" (
    "%ProgramFiles%\Apache\maven\bin\mvn.cmd" %MAVEN_CMD_LINE_ARGS%
    if ERRORLEVEL 1 goto error
    goto end
)
if exist "%ProgramFiles(x86)%\Apache\maven\bin\mvn.cmd" (
    "%ProgramFiles(x86)%\Apache\maven\bin\mvn.cmd" %MAVEN_CMD_LINE_ARGS%
    if ERRORLEVEL 1 goto error
    goto end
)
echo.
echo Error: Maven not found on your system. Please install Maven or set M2_HOME. >&2
echo.
goto error
